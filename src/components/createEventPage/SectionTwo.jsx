...
const SectionTwo = ({ ref }) => {
  ...
  return (
    <div
      ref={ref}
      className="h-[310px] w-full shadow relative flex mt-10 overflow-hidden"
    >
          ...
          <div className="h-full w-full bg-amber-200 relative">
            ...
            {createEventFormData?.headingImage && (
              <div className="absolute left-0 top-0 h-full overflow-hidden w-full bg-white flex justify-center items-center flex-col">
                <img
                  src={createEventFormData?.headingImage}
                  className="h-full w-full object-cover"
                  alt=""
                />
              </div>
            ...
            <input
              onChange={(evt) => handleEventInputfiledsChanges(evt)}
              required={!createEventFormData?.headingImage}
              type="file"
              className="opacity-0 h-full w-full cursor-pointer relative z-20"
              name="headingImage"
              accept="image/*"
            />
          </div>
        ...
        <div className="w-full relative z-10 flex justify-end">
          <div className="w-[90%]">
            <div className="h-[40px]">
              <textarea
                onInput={(e) => {
                  handleEventInputfiledsChanges(e);
                  autoResize(e);
                }}
                required
                value={createEventFormData?.heading}
                placeholder="HEADING"
                className="w-full outline-0 border-0 text-white placeholder:text-white text-[31px] font-light resize-none overflow-hidden"
                name="heading"
              />
            </div>
            <div className="min-h-[30px] mt-2 max-h-[90px]">
              <textarea
                ...
                required
                value={createEventFormData?.subHeading}
                placeholder="Sub Heading"
                className="w-[80%] outline-0 border-0 text-white placeholder:text-white text-[13px] leading-5 font-light resize-none overflow-hidden"
                name="subHeading"
              />
            </div>
          </div>
