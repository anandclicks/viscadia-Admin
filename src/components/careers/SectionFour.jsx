      ...
      <div className='flex flex-col justify-center items-center w-full'>
        {createCareerData?.qualifications?.map((el, minIndex) => (
          <div key={minIndex} className='flex flex-col justify-center items-center w-full'>
            <textarea required name="subHeading" value={el?.subHeading} onInput={(e) => handleObjInpusChanges(e, "qualifications", minIndex)} placeholder='Enter Sub Heading' className='placeholder:text-[21px] w-full text-start text-[21px] h-[40px] placeholder:text-black placeholder:text-start generalCssForInputs' />
            <div className='w-full flex flex-col justify-center items-start'>
              {el?.keyPoints?.map((text, index) => (
                <div key={index} className='flex justify-start gap-2 w-full'>
                  ...
                  <textarea required name="text" value={text} onInput={(e) => handleTextsChange(e, "qualifications", minIndex, index)} placeholder="Key Points" className="h-[24px] w-full outline-0 border-0 resize-none overflow-hidden placeholder:text-[#000] text-[#000] text-[14px]" />
                </div>
              ))}
