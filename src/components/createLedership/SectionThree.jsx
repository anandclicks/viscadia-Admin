...
const SectionThree = () => {
  ...
  const {createLeadershipData,handleSectionThreeObjInputs,addKeysInObjsTwo} = useContext(LeadershipContext)
  return (
          ...
          <div key={index} className="h-full">
            <div className="h-[150px] w-[150px] rounded-full bg-[#FFF8F8] overflow-hidden border border-dashed border-[#BD2F2C]">
              <div className="flex h-full w-full relative flex-col items-center justify-center">
                ...
                />
                {el?.img && <img className="h-full w-full object-cover" src={el?.img}/>}
               {!el?.img &&  <div className="h-full w-full flex flex-col justify-center items-center">
            ...
            <div className="mt-3">
              ...
              <div>
                {/* Static key points */}
             {createLeadershipData?.sectionThree[index].subHeading?.map((key,keyIndex)=>(
                ...
                <button onClick={()=> addKeysInObjsTwo("sectionThree",index)} className="bg-[#BD2F2C] mt-2 text-[#fff] p-[4px] rounded-full font-medium px-4 text-[13px]">
