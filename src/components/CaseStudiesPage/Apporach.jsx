...
const Apporach = () => {
    const {addObjsInSectionsTwo,createCaseStudyData} = useContext(NewCaseStudyContext)
  return (
     <div className="min-h-[500px] py-10 my-10 w-full shadow">
      ...
      <div className="grid grid-cols-3 gap-20 mt-10 min-h-[350px] pt-10 px-5">
       {createCaseStudyData?.approach?.map((el,index)=>(
       ...
       <div className="flex justify-center w-full">
          <button onClick={()=> addObjsInSectionsTwo()} className="bg-[#BD2F2C] text-[#fff] p-[6px] rounded-full font-medium px-4 text-[17px] ">
              <i className="ri-add-line"></i> Add More
            </button>
      </div>
    </div>
