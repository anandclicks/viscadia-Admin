...
function formatSubmissions(data) {
  ...
  if (!tableData) return <PageBuildingLoader />;
  if (filteredData.length === 0) return <NoDataFound message="No submissions found" />;
  ...
  return (
    <>
      ...
      <div className="border-[1px] border-stone-200 rounded-[20px] fsTwo overflow-x-auto">
          ...
          <tbody>
            {filteredData.map((el, index) => (
              <tr key={index} className="border-t border-gray-200">
                ...
                <td className="px-4 py-4 text-[14px] font-medium text-black">{`${el?.form_data?.name || ""} ${el?.form_data?.first_name || ""} ${el?.form_data?.last_name || ""}`}</td>
                <td className="px-4 py-4 text-[14px] font-medium text-black">{el?.form_data?.email || "NA"}</td>
                <td className="px-4 py-4 text-[14px] font-medium text-black"><span className="truncate block max-w-[150px]" title={el?.form_data?.company || "NA"}>{el?.form_data?.company || "NA"}</span></td>
                <td className="px-4 py-4 text-[14px] font-medium text-black"><span className="truncate block max-w-[150px]" title={el?.form_data?.message || "NA"}>{el?.form_data?.message || "NA"}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
