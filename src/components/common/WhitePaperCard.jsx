...
const WhitePaperCard = ({ id, isOpen, onToggle, data }) => {
  ...
  const handleStatusApiCall = async (status) => {
    ...
    let res = await putCommonApiForEvnts(`/whitepaper/${data?.id}`, { status });
    if (res.success) {
      toast.dismiss(t);
    ...
    <>
      {data && (
        <div
          ...
          className="w-full min-h-[200px] my-5 rounded-[30px] flex fsTwo shadow-[0px_0px_3px_#0000000f] border border-[#f1f1f1]"
        >
          <img
            className="w-[28%] max-h-[230px] rounded-[30px] object-cover"
            ...
            <div className="w-full flex justify-between mb-2">
              ...
              <div className="relative flex gap-3 mb-2">
                ...
                >
                  ...
                  {/* <Link to={`/preview/white-paper/${data?.id}`}>
